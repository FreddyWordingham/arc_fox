//! Optical gate information structure.

use nalgebra::{Unit, Vector3};

/// Optical surface traversal information structure.
pub struct Gate {
...
impl Gate {
    ...
    pub fn new(
        ...
        let norm = if inc.dot(face_norm) > 0.0 {
            Unit::new_unchecked(face_norm.into_inner() * -1.0)
        } else {
            Unit::new_unchecked(face_norm.into_inner())
        };
