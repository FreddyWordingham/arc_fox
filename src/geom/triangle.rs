...
impl Triangle {
    ...
    /// Generate a point on the surface within the aabb.
    #[pre(self.overlap(&aabb))]
    #[post(aabb.contains(&ret))]
    pub fn point_on_surf_within(&self, aabb: &Aabb) -> Point3<f64> {
        let mut tri_rays = Vec::with_capacity(3);
        let mut aabb_rays = Vec::with_capacity(12);
        for i in 0..3 {
            let j = (i + 1) % 3;
            let k = (i + 2) % 3;

            tri_rays.push(Ray::new(
                self.verts[i],
                Unit::new_normalize(self.verts[j] - self.verts[i]),
            ));
            tri_rays.push(Ray::new(
                self.verts[i],
                Unit::new_normalize(self.verts[k] - self.verts[i]),
            ));

            let mut axis = Vector3::new(0.0, 0.0, 0.0);
            axis[i] = 1.0;
            let axis = Unit::new_normalize(axis);
            aabb_rays.push(Ray::new(aabb.mins().clone(), axis));
            aabb_rays.push(Ray::new(aabb.maxs().clone(), -axis));

            let mut shift = Vector3::new(0.0, 0.0, 0.0);
            shift[j] = aabb.widths()[j];
            aabb_rays.push(Ray::new(aabb.mins() + shift, axis));
            aabb_rays.push(Ray::new(aabb.maxs() - shift, -axis));
        }

        let mut points = Vec::new();
        for ray in tri_rays {
            if let Some(p) = aabb.hit_point(&ray) {
                points.push(p);
            }
        }
        for ray in aabb_rays {
            if let Some(p) = self.hit_point(&ray) {
                points.push(p);
            }
        }
        ...
        if points.len() < 3 {
            panic!("Not enough hit points!");
        }

        for p0 in points.iter() {
            for p1 in points
                .iter()
                .filter(|p| nalgebra::distance(*p, p0) > (1.0e3 * EPSILON))
            {
                for p2 in points
                    .iter()
                    .filter(|p| nalgebra::distance(*p, p0) > (1.0e3 * EPSILON))
                    .filter(|p| nalgebra::distance(*p, p1) > (1.0e3 * EPSILON))
                {
                    return Point3::new(
                        (p0.x + p1.x + p2.x) / 3.0,
                        (p0.y + p1.y + p2.y) / 3.0,
        ...
        unreachable!("Not enough hit point or they weren't adequately separated.");
    }
}
